...
One failure is neither, and is the reason for the whole classification: the
**claude session limit**. It belongs to the account, not to the session, so
the attempt is not repeated and the issue is not escalated — it keeps its
state label and the whole factory stops dispatching until the limit resets.
See [The claude session limit](configuration.md#the-claude-session-limit).
