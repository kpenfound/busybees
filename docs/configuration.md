...
`~/.config/bees/config.toml` and the project's `context.toml`, with
`defaults.toml` below the first; [Reviewing a pull
request](review.md#configuration) describes all three.
...
that, with `max_cost_per_day` set, pauses dispatch like a budget reached:
nothing the daily budget gates starts until the ledger reads again, and
`bees status` shows `paused: ledger unreadable: <file> line <n> does not
parse`. With `max_cost_per_issue` set, an issue whose spend has to be seeded
from the ledger at that moment is escalated the same way an issue over the
budget is, with the escalation naming the line. With neither set the line
fails `bees cost` and the `today:` line of `bees status` alone. Remove or
repair the line to resume; a torn final line, what a crash mid-write leaves,
is ignored. See
...
environment when it is set there, the role's `env` (which may name the
credential itself), `SHELL`, the `BEES_*` variables, the
[`[github]`](#github) token and git identity, the git
...
The credential is the agent's own: `ANTHROPIC_API_KEY` or
`CLAUDE_CODE_OAUTH_TOKEN` for claude, `OPENAI_API_KEY` or `CODEX_API_KEY`
for codex, and for opencode and pi the key of whichever provider the model
names. opencode reads `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`,
`GEMINI_API_KEY`, `GOOGLE_API_KEY`, `OPENROUTER_API_KEY`, `GROQ_API_KEY`,
`MISTRAL_API_KEY`, `XAI_API_KEY` and `DEEPSEEK_API_KEY`; pi's keys are
listed under [Pi](#pi). Every one of them that is set is forwarded.
