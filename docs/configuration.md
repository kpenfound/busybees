...
| `stacked_prs` | bool | `false` | Build a `blocked_by` chain of work items under the same feature as a stack of branches and pull requests, each depending on its predecessor's, instead of independently. `false` keeps every work item branching from the default branch on its own, as it does today. |
