...
#retry_delay = "10m"
# Run the retry on the profile the role's profile names as its fallback.
...
# Let a role record a draft issue about an error the factory itself caused (a
# tool, a prompt, the orchestrator), scrubbed of repository names, tokens, paths
# and people, under <state_dir>/feedback/ for filing upstream.
...
#sandbox = "none"       # none | claude | container | sbx (pi: none | container)
...
#skills_refresh = "24h"
# Pi packages a pi session loads on top of pi-mcp-adapter, which every pi
# session loads: npm:<name>[@version], git:<repo>[@ref], a URL or a path.
# Global and role lists are unioned; other agents ignore them.
...
# For sandbox = "sbx" it is the sandbox's template, built on sbx's image for
# the agent (docker/sandbox-templates:claude-code, :codex or :opencode);
...
# The host's Dagger engine an sbx session is given (sandbox = "sbx" only),
# with the Dagger CLI at sandbox_dagger_version, the engine's own release,
# installed in the sandbox. Off unless set: it widens what a session reaches.
