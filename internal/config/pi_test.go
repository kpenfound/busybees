...
`))
	...
	if dev.Agent != AgentPi || dev.Model != "" || dev.Sandbox != SandboxNone {
...
// Pi has no sandbox of its own and sbx has no template for it, so a profile
// asking for Claude Code's sandbox or sbx with agent = "pi" is refused at
// load, naming the key, however the profile came to be: declared, or
// migrated from a version 1 file's role settings.
func TestPiRefusesOtherSandboxesAtLoad(t *testing.T) {
	for _, tc := range []struct{ body, key, mode string }{
		{"version = 5\n[profiles.remote]\nagent = \"pi\"\nsandbox = \"claude\"\n", "profiles.remote.sandbox", SandboxClaude},
		{"version = 5\n[profiles.default]\nagent = \"pi\"\nsandbox = \"claude\"\n", "profiles.default.sandbox", SandboxClaude},
		{"version = 5\n[profiles.boxed]\nagent = \"pi\"\nsandbox = \"sbx\"\n", "profiles.boxed.sandbox", SandboxSbx},
		{"version = 1\n[project]\nrepo = \"a/b\"\n[global]\nsandbox = \"claude\"\n[roles.qa]\nagent = \"pi\"\n", ".sandbox", SandboxClaude},
	} {
		_, err := Load(writeConfig(t, tc.body))
		if err == nil {
			t.Errorf("%q loaded", tc.body)
			continue
		}
		for _, want := range []string{tc.key, `"` + tc.mode + `"`, `"pi"`, "none or container"} {
			if !strings.Contains(err.Error(), want) {
				t.Errorf("%q: error %q does not mention %s", tc.body, err, want)
			}
		}
	}
	...
	if err := CheckSandboxAgent(SandboxSbx, AgentPi); err == nil || !strings.Contains(err.Error(), `"pi"`) {
		t.Errorf("CheckSandboxAgent(sbx, pi) = %v", err)
	}
