	...
	case config.RoleDeveloper, config.RoleReviewer:
		...
		if role == config.RoleReviewer {
			...
			if s.stateOf(i.Labels) != "review" {
				...
				// Every state label has to go, not just the one stateOf
				// names: an issue a person held carries bees:needs-human on
				// top of the state label underneath, and leaving that one
				// behind makes stateOf read it back out of the copy.
				i.Labels = setStateLocal(i.Labels, s.labels.StateLabels(), s.labels.Review)
			}
