...
func (c *Client) TagExists(ctx context.Context, tag string) (bool, error) {
	out, err := c.Exec(ctx, "api", fmt.Sprintf("repos/%s/git/matching-refs/tags/%s", c.Repo, tag))
	if err != nil {
		return false, err
	}
	var refs []struct {
...
// CreateTag creates a repository tag at commit. GitHub's refs API writes the
// remote ref directly, equivalent to creating and pushing a lightweight tag.
func (c *Client) CreateTag(ctx context.Context, tag, commit string) error {
	_, err := c.Exec(ctx, "api", "--method", "POST", fmt.Sprintf("repos/%s/git/refs", c.Repo),
		"-f", "ref=refs/tags/"+tag, "-f", "sha="+commit)
	return err
}

// BranchHead reads the commit currently at a branch on the remote repository.
func (c *Client) BranchHead(ctx context.Context, branch string) (string, error) {
	out, err := c.Exec(ctx, "api", fmt.Sprintf("repos/%s/git/ref/heads/%s", c.Repo, branch))
	if err != nil {
		return "", err
	}
	var ref struct {
	...
	if ref.Object.SHA == "" {
		return "", errors.New("branch ref has no commit")
	}
	return ref.Object.SHA, nil
}

// CreateRelease publishes a release with notes generated by GitHub.
func (c *Client) CreateRelease(ctx context.Context, tag string) error {
	_, err := c.Exec(ctx, "release", "create", tag, "-R", c.Repo, "--generate-notes")
	return err
}

// CloseMilestone closes a milestone by its number.
func (c *Client) CloseMilestone(ctx context.Context, number int) error {
	_, err := c.Exec(ctx, "api", "--method", "PATCH", fmt.Sprintf("repos/%s/milestones/%d", c.Repo, number), "-f", "state=closed")
	return err
}
