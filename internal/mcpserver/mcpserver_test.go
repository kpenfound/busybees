...
func TestSessionToolsAreOfferedToEveryRole(t *testing.T) {
	for _, role := range append(config.Roles, "") {
		...
		for _, want := range []string{"done", "issue_create", "issue_link", "mail_list", "mail_send", "notes_read", "notes_write", "report_factory_error"} {
...
// TestAGENTSMdListsEveryTool pins the `internal/mcpserver` bullet in AGENTS.md —
...
func TestAGENTSMdListsEveryTool(t *testing.T) {
	...
	if line == "" {
		t.Fatalf("AGENTS.md: no line starts with %q. That bullet lists the built-in MCP tools and is kept in sync with internal/mcpserver; restore it (or update this test) rather than leaving the layout map without one.", bullet)
	}
	...
	if len(missing) > 0 {
		t.Errorf("AGENTS.md: the %q line does not list %s, registered by the server for at least one role (internal/mcpserver/mcpserver.go). Add the missing names to that line:\n%s",
			bullet, strings.Join(missing, ", "), line)
	}
	if len(extra) > 0 {
		t.Errorf("AGENTS.md: the %q line lists %s, not a tool the server registers for any role. Drop the stale names from that line — or, if one is a word that merely happens to be backticked there, unbacktick it:\n%s",
