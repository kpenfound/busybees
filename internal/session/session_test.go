...
`)
	...
	r.CodexBin = bin
	sessions := r.SessionsDir
	role := config.ResolvedRole{Name: "qa", Agent: config.AgentCodex, Timeout: time.Minute, Sandbox: config.SandboxClaude}
	_, err := r.Run(context.Background(), Request{Name: "boxed-codex", Role: role, WorkDir: t.TempDir(), SystemPrompt: "SYS", Prompt: "TASK"})
	if err == nil {
		t.Fatal("a codex session ran in a claude box")
	}
	for _, want := range []string{"qa", "codex", "claude"} {
		if !strings.Contains(err.Error(), want) {
			t.Errorf("error %q does not mention %q", err, want)
		}
	}
	...
	for _, e := range entries {
		if _, err := os.Stat(filepath.Join(sessions, e.Name(), "ran")); err == nil {
			t.Error("codex ran for a session bees refused to box")
		}
	}
}
