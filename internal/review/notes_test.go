...
// dismiss appends one dismissal of testRepo, from the general angle, and fails
...
func TestADismissalIsAppendedToTheNotes(t *testing.T) {
	...
	dismiss(t, path, AngleGeneral, "naming", "receiver names are short here")
	...
	want := []Dismissal{
		...
		{Repo: testRepo, Angle: AngleGeneral, Category: "naming", Reason: "receiver names are short here"},
	}
...
` + rulesBegin + `
- [acme/widgets] [general] [naming] drop: receiver names are short here (4 dismissals)
- [*] [*] [*] downrank: whatever it is
[acme/widgets] [general] [naming] dropp: a misspelt action
- [acme/widgets] [general] [naming] a rule with no action at all
` + rulesEnd + `
...
`
	...
	wantRules := []Rule{
		{Repo: testRepo, Angle: AngleGeneral, Category: "naming", Action: RuleDrop, Text: "receiver names are short here", Count: 4},
...
func TestARuleIsWrittenAsItIsRead(t *testing.T) {
	...
	// one.
	if got := (Rule{Angle: AngleGeneral, Action: RuleDrop, Text: "x", Count: 1}).Line(); got != "- [*] [general] [*] drop: x (1 dismissal)" {
...
func TestConsolidateMakesARuleOfAPatternThatRepeats(t *testing.T) {
	...
	// one of a third.
	dismiss(t, path, AngleGeneral, "naming", "receiver names are short here")
	dismiss(t, path, AngleGeneral, "naming", "short receiver names are fine here")
	dismiss(t, path, AngleGeneral, "naming", "receiver names here are short")
	...
	want := []Rule{
		{Repo: testRepo, Angle: AngleGeneral, Category: "naming", Action: RuleDrop, Text: "receiver names are short here", Count: 3},
...
func TestAPatternOfAnotherRepoAngleOrCategoryIsAnotherPattern(t *testing.T) {
	...
	// pattern repeats.
	dismiss(t, path, AngleGeneral, "naming", "not how we write it")
	dismiss(t, path, AngleGeneral, "wording", "not how we write it")
	dismiss(t, path, AngleTests, "naming", "not how we write it")
	if err := AppendDismissal(path, Dismissal{Repo: "acme/gadgets", Angle: AngleGeneral, Category: "naming", Reason: "not how we write it"}); err != nil {
		t.Fatal(err)
	}
	n, err := ReadNotes(path)
	if err != nil {
	...
	if len(added) != 1 || added[0].Angle != AngleGeneral || added[0].Category != "naming" || added[0].Repo != testRepo {
		t.Errorf("added %+v, want the one rule for %s/%s/naming", added, testRepo, AngleGeneral)
	}
}
...
func TestConsolidateKeepsTheRuleAPersonWroteAndRefreshesItsCount(t *testing.T) {
	...
	n.Rules = []Rule{{Repo: testRepo, Angle: AngleGeneral, Category: "naming", Action: RuleDownrank, Text: "short receiver names are the convention here", Count: 2}}

	added, refreshed := n.Consolidate()
	if len(added) != 0 {
		t.Errorf("added %+v, want none: the pattern is already ruled on", added)
	}
	want := []Rule{{Repo: testRepo, Angle: AngleGeneral, Category: "naming", Action: RuleDownrank, Text: "short receiver names are the convention here", Count: 3}}
...
func TestARuleWithNoTextRulesOnItsWholeCategory(t *testing.T) {
	...
	n.Rules = []Rule{{Repo: anyValue, Angle: AngleGeneral, Category: "naming", Action: RuleDrop}}
	added, refreshed := n.Consolidate()
...
func TestWritingTheRulesLeavesEveryOtherLineAlone(t *testing.T) {
	...
	n.Rules = []Rule{{Repo: testRepo, Angle: AngleGeneral, Category: "naming", Action: RuleDrop, Text: "receiver names are short here", Count: 3}}
...
func TestNotesWithNoBlockGetOne(t *testing.T) {
	...
	n.Rules = []Rule{{Repo: testRepo, Angle: AngleGeneral, Category: "naming", Action: RuleDrop, Text: "receiver names are short here"}}
	if err := n.Write(); err != nil {
		t.Fatal(err)
	}
	want := "my own notes\n\n" + rulesHeading + "\n\n" + rulesBegin + "\n" +
		"- [acme/widgets] [general] [naming] drop: receiver names are short here\n" + rulesEnd + "\n"
	if got := readFile(t, path); got != want {
		t.Errorf("notes =\n%q\nwant\n%q", got, want)
	}
}
...
func TestDismissalsThatReadNothingAlikeAreNotOnePattern(t *testing.T) {
	...
	dismiss(t, path, AngleGeneral, "naming", "receiver names are short here")
	dismiss(t, path, AngleGeneral, "naming", "the migration guide covers it")
	dismiss(t, path, AngleGeneral, "naming", "nobody reads that log line")
	n, err := ReadNotes(path)
	if err != nil {
		t.Fatal(err)
	}
