...
mid-run: the toolchain (git, gh, claude, and codex, opencode or pi when a
role is configured with that agent), the configuration, access to the GitHub
repository and creating a worktree.
...
and it never changes bees.toml.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			d := &doctor.Deps{ClaudeBin: claudeBin(), CodexBin: codexBin(), OpenCodeBin: opencodeBin(), PiBin: piBin()}
			...
			if path, err := configPath(g); err != nil {
				...
				d = doctor.New(cmd.Context(), path, d.ClaudeBin, d.CodexBin, d.OpenCodeBin, d.PiBin)
			}
			checks := d.Checks()
