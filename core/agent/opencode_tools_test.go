...
// environment, marks that it launched, and runs two tools the way opencode
// would under the permissions it was handed: edit writes edited.txt in the
// working directory, bash touches a marker, and a tool the permissions do
// not allow ends in an error instead.
...
// grantedServer is the session's own MCP server in these tests.
var grantedServer = MCPEntry{Command: "/usr/local/bin/task", Args: []string{"mcp", "serve"}}
...
func TestOpenCodeWritableTurnIsHeldToItsGrantedTools(t *testing.T) {
	for _, pl := range grantedPlacements() {
		t.Run(pl.where.String(), func(t *testing.T) {
			...
			// The granted tool ran and wrote into the working directory;
			// the one not granted did not run.
			...
			for _, want := range []string{`"tool":"edit","state":{"status":"completed"}`, `"tool":"bash","state":{"status":"error"}`} {
			...
			if own := content.MCP["tools"]; !own.Enabled || !slices.Equal(own.Command, []string{grantedServer.Command, "mcp", "serve"}) {
