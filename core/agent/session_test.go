...
func TestClaudeCommandResumes(t *testing.T) {
	...
	for _, id := range []string{"", "abc-123"} {
		...
		_, args, _, _, err := claudeBackend{}.command(context.Background(), r.Runner, Request{Name: "n", Profile: role, Prompt: "TASK", ResumeID: id}, paths)
		if err != nil {
			t.Fatal(err)
		}
...
func TestCodexCommandIgnoresResume(t *testing.T) {
	...
	var got [][]string
	for _, id := range []string{"", "abc-123"} {
		_, args, stdin, _, err := codexBackend{}.command(context.Background(), r.Runner, Request{Name: "n", Profile: codexRole("gpt-5"), SystemPrompt: "SYS", Prompt: "TASK", ResumeID: id}, paths)
		if err != nil {
			t.Fatal(err)
		}
		if stdin != "SYS\n\n---\n\nTASK" {
