...
// context bundle and the change it is about are appended to it.
//
...
type Distiller[R Reference] struct {
	...
	// none, and the session then runs in an empty
...
// nothing the caller already knows: the reference, the sources gathered and
...
func (d *Distiller[R]) Distill(ctx context.Context, b *Bundle[R]) (*Brief[R], error) {
	if b == nil {
		return nil, errors.New("distill: no context bundle")
	}
	dir := d.Dir
	if dir == "" {
		tmp, err := os.MkdirTemp("", "review-")
		if err != nil {
			return nil, err
		}
	...
	res, err := d.Agent.Run(ctx, AgentRequest{Name: DistillerName, Prompt: distillPrompt[R](b), Dir: dir})
	if err != nil {
		return nil, err
	}
	brief, err := ParseBrief[R](res.Text)
	if err != nil {
...
func distillPrompt[R Reference](b *Bundle[R]) string {
...
func ParseBrief[R Reference](text string) (*Brief[R], error) {
