...
// and the reviewer notes (policy.go) are where the dismissals they have made
...
// opaque scope used to select rules.
//
...
func Filter(findings []Finding, rules []Rule, repo string, compare Comparator) ([]Finding, []Silenced) {
	kept := make([]Finding, 0, len(findings))
	var silenced []Silenced
	for _, f := range findings {
		rule, ok := matchRule(rules, repo, &f, compare)
		if !ok {
			kept = append(kept, f)
			continue
		}
...
// acts.
func matchRule(rules []Rule, repo string, f *Finding, compare Comparator) (Rule, bool) {
	var best Rule
	found := false
	for _, r := range rules {
		if !r.Matches(repo, f, compare) {
			continue
		}
...
func (r Rule) Matches(repo string, f *Finding, compare Comparator) bool {
	if !matchesPattern(r.Scope, repo) || !matchesPattern(r.Angle, f.Angle) || !matchesPattern(r.Category, f.Category) {
		return false
	}
	if strings.TrimSpace(r.Text) == "" {
		return true
	}
	return compare != nil && compare(r.Text, "", f.Title, "")
}
...
func NoiseSection(rules []Rule, repo, angle string) string {
	...
	for _, r := range rules {
		if !matchesPattern(r.Scope, repo) || !matchesPattern(r.Angle, angle) || strings.TrimSpace(r.Text) == "" {
			continue
		}
		if out.Len() == 0 {
