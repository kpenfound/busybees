...
func testBundle() *Bundle[testRef] {
	return &Bundle[testRef]{
		Ref:   testRef{Key: "change-7", Scope: testRepo, Link: "https://example.test/change-7"},
		Title: "widgets: gather the context", Author: "octocat",
		Items: []Item{
			{Source: SourceDiff, Name: "change-7", Content: "diff --git a/gather.go b/gather.go"},
			{Source: SourceStyleFiles, Name: "CLAUDE.md", Content: "Every new key needs a test."},
		},
		Skipped: []string{"#99 is mentioned by change-7 and is not an issue that could be read"},
	}
}
...
func TestTheDistillerReadsTheBundleAndWritesTheBrief(t *testing.T) {
	...
	want := &Brief[testRef]{
		...
		NotGathered:        []string{"#99 is mentioned by change-7 and is not an issue that could be read"},
		SessionID:          "sess-1",
		CostUSD:            0.5,
	}
...
func TestTheFactsInTheBriefComeFromTheBundle(t *testing.T) {
	...
	if brief.Ref != (testRef{Key: "change-7", Scope: testRepo, Link: "https://example.test/change-7"}) {
...
func TestTheDistillerSessionIsToldWhatToDoAndGivenTheBundle(t *testing.T) {
	...
	if _, err := (&Distiller[testRef]{Agent: agent, Dir: t.TempDir()}).Distill(context.Background(), testBundle()); err != nil {
		t.Fatal(err)
	}
...
func TestWithoutACheckoutTheSessionRunsInAnEmptyDirectory(t *testing.T) {
	agent := &fakeAgent{answer: answeredBrief}
	if _, err := (&Distiller[testRef]{Agent: agent}).Distill(context.Background(), testBundle()); err != nil {
		t.Fatal(err)
	}
...
func TestTheDistillerRunsInTheCheckout(t *testing.T) {
	dir := t.TempDir()
	agent := &fakeAgent{answer: answeredBrief}
	if _, err := (&Distiller[testRef]{Agent: agent, Dir: dir}).Distill(context.Background(), testBundle()); err != nil {
		t.Fatal(err)
	}
...
func TestASessionThatFailedProducesNoBrief(t *testing.T) {
	...
	_, err := (&Distiller[testRef]{Agent: agent, Dir: t.TempDir()}).Distill(context.Background(), testBundle())
...
func TestAnAnswerThatIsNotABriefIsAnError(t *testing.T) {
	for _, tc := range []struct{ name, answer, want string }{
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if err != nil && !strings.Contains(err.Error(), "change-7") {
...
func TestDistillingWithoutABundle(t *testing.T) {
	agent := &fakeAgent{answer: answeredBrief}
	if _, err := (&Distiller[testRef]{Agent: agent}).Distill(context.Background(), nil); err == nil {
...
func TestASessionCannotAnswerWithWhatItWasNotAsked(t *testing.T) {
	...
	brief, err := ParseBrief[testRef](`{"summary": "s", "size": "xs", "ref": {"repo": "evil/repo", "number": 1},
		"title": "a title of its own", "author": "someone else", "session_id": "made up",
		"sources": ["invented"], "not_gathered": ["nothing at all"]}`)
	if err != nil {
		t.Fatal(err)
	}
	if brief.Ref != (testRef{}) || brief.Title != "" || brief.Author != "" || brief.SessionID != "" || brief.Sources != nil || brief.NotGathered != nil {
