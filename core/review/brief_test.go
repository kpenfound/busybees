...
func testBrief() *Brief[testRef] {
	return &Brief[testRef]{
...
func TestABriefWithNothingInItIsTwoLines(t *testing.T) {
	...
	// did not.
	got := (&Brief[testRef]{Ref: testRef{Key: "change-7", Scope: testRepo, Link: "https://example.test/change-7"}}).Text()
	want := "# Review brief: change-7\n\nhttps://example.test/change-7\n"
	if got != want {
		t.Errorf("brief =\n%q\nwant\n%q", got, want)
	}
}
...
func TestABriefWithNothingInASectionHasNoSuchHeading(t *testing.T) {
	brief := &Brief[testRef]{Ref: testRef{Key: "change-7", Scope: testRepo, Link: "https://example.test/change-7"}, Summary: "a small change"}
	got := brief.Text()
...
func TestABriefWithoutASummaryIsNotABrief(t *testing.T) {
	for _, summary := range []string{"", " \n\t"} {
		brief := &Brief[testRef]{Ref: testRef{Key: "change-7", Scope: testRepo, Link: "https://example.test/change-7"}, Summary: summary, StyleRules: []Point{{Text: "a rule"}}}
...
func TestReadingABriefThatIsNotThere(t *testing.T) {
	if _, err := ReadBrief[testRef](t.TempDir()); err == nil {
