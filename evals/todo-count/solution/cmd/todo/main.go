...
//	todo [-f file] count        print how many items are pending and done
package main
...
func run(args []string, out io.Writer) error {
	...
	args = fs.Args()
	if len(args) == 0 {
		return errors.New("usage: todo [-f file] list|add|done|count")
	}
	l, err := load(*file)
	if err != nil {
		return err
	...
	case "count":
		if len(args) != 1 {
			return errors.New("usage: todo count")
		}
		pending := len(l.Pending())
		fmt.Fprintf(out, "%d pending, %d done\n", pending, len(l)-pending)
		return nil
	case "add":
