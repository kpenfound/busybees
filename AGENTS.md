...
- Tests must never call the real `claude`, `codex`, `opencode`, `pi`, `gh`, `docker` or `sbx`. `gh` is faked through
  ...
  `exec` and `eval` refuse to start inside a session (`BEES_SESSION_DIR` set) for the same reason.
...
- `cmd/bees` — cobra CLI (`init`, `run`, `tick`, `exec`, `status`, `cost`, `machine`, `kill`, `mail`, `notes`, `issue`, `done`, `mcp`, `doctor`, `config`, `skills`, `prompts`, `labels`, `templates`, `review`, `eval`, `version`).
...
- `internal/fakegh` — an in-memory GitHub behind `github.Client.Exec` (`New`, `GitHub.Exec`): seeded from data (`Load`: issues with labels, milestone, parent, author and comments; pull requests with comments and reviews; milestones; labels) or through its exported fields, read back with `Snapshot` (issues, pull requests, comments posted, label history, merges) and `CallCount`/`Total`; writes a session process asks for arrive through a directory (`RequestEdit`, `EditsDir`), or through `Exec` and `ExecStdin` from the gh `internal/eval` puts on a session's PATH. The scheduler tests drive the factory against it (`fakeGH` in `internal/scheduler/scheduler_test.go` adds their PR visibility and implicit parent).
- `internal/eval` — `bees eval`, the whole-factory eval, and `bees eval <role>`, the per-role one: cases under `evals/<case>/` and `evals/<role>/<case>/` (`case.go`: `case.toml`, the fixture in `repo/` or built by `setup.sh`, a seeded pull request's head branch as a tree in `pr/<head>`, hidden test files in `grade/`, `LoadCases` taking neither the role directories, which `LoadRoleCases` reads one of, nor `evals/fixtures/`, which is no case at all and holds the fixtures the `todo-*` cases' `setup.sh` copy in; a whole-factory case is graded by its `test` and a per-role one by its `[expect]`, and `validate` refuses each other's keys; `shipped_test.go` checks every shipped whole-factory case fails on its fixture and passes with its `solution/`, and that every per-role case loads and builds), the profiles the sessions run on (`profile.go`: `SelectProfile` from `--profile`, `bees.toml`'s selection or the global `config.toml`, written into an eval `bees.toml` that carries nothing else of the file they came from, only the eval's own settings — the reviewer's checks timings, and for a per-role reviewer case one review round (`ReviewRounds`), so that the verdict ends the run instead of the loop going on to a developer session — and refuses container and sbx sandboxes), the gh every session gets first on its PATH (`shim.go`: a script running `bees eval gh <url>`, which forwards the call to the case's `fakegh` over loopback HTTP), the run (`run.go`: fixture as a bare origin, then either scheduler passes with `Once`, approved pull requests merged between them, stopped by done, timeout or budget and graded by the case's test on the default branch, or one `Scheduler.RunRole` for a per-role case, scoped by `Scheduler.OnlyRoles` and not by `roles.<name>.enabled`, so reconcile routes new issues the way the configured factory does; both bounded by the same `watcher`), the mechanical checks of a per-role case (`expect.go`: the outcome from the ledger, labels, mail the role sent, issues created or closed, a pull request opened), the grader sessions its rubrics are scored by (`grader.go`: `Runner.Grader`, a `review.Agent` `cmd/bees` builds from the global `config.toml` and never from `--profile`, told the rubric, the run's transcripts and the end state, answering a score and reasons) and the table and `report.json` (`report.go`). Its tests run the whole loop with a fake claude, a fake grader and fake project manager and QA sessions in `TestMain`.
