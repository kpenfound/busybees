...
- `internal/eval` — `bees eval`, the whole-factory eval: cases under `evals/<case>/` (`case.go`: `case.toml`, the fixture in `repo/` or built by `setup.sh`, hidden test files in `grade/`; `evals/<role>/` is left for per-role cases and `evals/fixtures/` holds the fixtures the `todo-*` cases' `setup.sh` copy in; `shipped_test.go` checks every shipped case fails on its fixture and passes with its `solution/`), the profiles the sessions run on (`profile.go`: `SelectProfile` from `--profile`, `bees.toml`'s selection or the global `config.toml`, written into an eval `bees.toml` that carries nothing else and refuses container and sbx sandboxes), the gh every session gets first on its PATH (`shim.go`: a script running `bees eval gh <url>`, which forwards the call to the case's `fakegh` over loopback HTTP), the run (`run.go`: fixture as a bare origin, scheduler passes with `Once`, approved pull requests merged between them, stopped by done, timeout or budget, then graded by the case's test on the default branch) and the table and `report.json` (`report.go`). Its tests run the whole loop with a fake claude in `TestMain`.
