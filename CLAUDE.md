...
- `internal/review` — the configuration surface of `bees review`, the pull request review tool: the global `~/.config/bees/config.toml` (`config.go`: provider/model, reviewer notes and review artifact paths, default output mode, GitHub token, TUI preferences) and the project's `context.toml` (`project.go`: which angles run, extra style sources, category severity overrides, which context sources are gathered). Neither file is `bees.toml`, neither is versioned or migrated, and a missing one loads as the defaults; a key the schema does not have, and a value outside its enum, are load errors naming the key.
