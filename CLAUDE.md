...
- `internal/session` — runs one `claude -p` session; outcome file written by `bees done`; `CheckInterrupted` says whether a session directory belongs to a session that is running, one that finished, or one a killed scheduler or a hard stop left unfinished.
