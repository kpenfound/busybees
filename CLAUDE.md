...
- `internal/duplicates` — `Find`: the existing issues a title and body would duplicate, scored by local word overlap over every issue in the repository, open and closed, whatever its labels (`github.Client.ListAllIssues` ignores `[filter]` on purpose). Explicitly invoked by the callers that file bugs (QA's `file_bug`, the upstream feedback loop's `drainFeedbackQueue`), never by `issues.Create`: a triage split and a feature's work items look alike on purpose.
