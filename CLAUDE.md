...
- `internal/procs` — pid files + `ps` scan to find and kill orphaned sessions (`bees kill`, and the live view's `k` key through `Scheduler.KillSession`); a container session is found through its container instead (`container.go`: the id file and the `bees.session` label, both named here and written by `internal/session`) and stopped by removing it.
