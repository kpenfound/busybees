...
- `internal/tui` — the live view `bees run` draws in a terminal (bubbletea + lipgloss): the Now and Queues panels, fed by `scheduler.Subscribe` and `status.json`. Drawn only when stdout is a terminal and `--no-tui` was not given (`tuiMode` in `cmd/bees`), and it silences console logging while it is up.
