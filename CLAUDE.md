...
- `internal/mail` — local JSON mailbox; the only channel between roles. `internal/feedback` — the queue of factory-error drafts `report_factory_error` writes (`Draft`, `Queue.Add`/`List`); removing a draft belongs to whatever files it upstream.
