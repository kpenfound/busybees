...
- `internal/tui` — the live view `bees run` draws in a terminal (bubbletea + lipgloss): the Now, Recent, Needs human, Approved PRs and Queues panels, fed by `scheduler.Subscribe` and `status.json`, plus its keys (arrows select, `o` opens on GitHub, `k` stops the selected session, `q`/ctrl-c drains). Drawn only when stdout is a terminal and `--no-tui` was not given (`tuiMode` in `cmd/bees`), and it silences console logging while it is up.
